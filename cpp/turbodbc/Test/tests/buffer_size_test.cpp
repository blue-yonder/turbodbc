...
TEST(BufferSizeTest, BufferSizeIsDefaultConstructible)
{
    ...
    EXPECT_EQ(1, std::get<turbodbc::rows>(one_row).value);
}
...
TEST(BufferSizeTest, DetermineBufferSizeWithRows)
{
    ...
    std::size_t rows_to_buffer = std::visit(turbodbc::determine_rows_to_buffer(descriptions), ten_rows);
...
TEST(BufferSizeTest, DetermineBufferSizeWithMegabytes)
{
    ...
    std::size_t rows_to_buffer = std::visit(turbodbc::determine_rows_to_buffer(descriptions), ten_mb);
...
TEST(BufferSizeTest, DetermineBufferSizeWithMegabytesWhereRowLargerThanDesiredBuffer)
{
    ...
    std::size_t rows_to_buffer = std::visit(turbodbc::determine_rows_to_buffer(descriptions), one_mb);
    EXPECT_EQ(1, rows_to_buffer);
}
...
TEST(BufferSizeTest, HalveBufferSizeWithRows)
{
    ...
    EXPECT_EQ(15 + 1, std::get<turbodbc::rows>(std::visit(turbodbc::halve_buffer_size(), odd)).value);
    EXPECT_EQ(21, std::get<turbodbc::rows>(std::visit(turbodbc::halve_buffer_size(), even)).value);
}
...
TEST(BufferSizeTest, HalveBufferSizeWithMegabytes)
{
    ...
    EXPECT_EQ(15 + 1, std::get<turbodbc::megabytes>(std::visit(turbodbc::halve_buffer_size(), odd)).value);
    EXPECT_EQ(21, std::get<turbodbc::megabytes>(std::visit(turbodbc::halve_buffer_size(), even)).value);
}